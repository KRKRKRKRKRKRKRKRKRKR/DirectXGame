...
//   任意で先頭に「R=3」「G=3」「B=3」のような取得目標数指定、「M=20」のような行動可能マス数
//   （GridBoardPlayerComponent::maxCost）指定を合わせて0〜4行（順不同・省略可）、
//   続けて7x7の文字グリッド（各文字はR=赤アイテム/G=緑アイテム/B=青アイテム/W=壁/0=空白マス）。
//   各指定を省略した項目は、読み込み時に対応する現在の設定（敵HPバーの目標数・maxCost）を変更しない
//
