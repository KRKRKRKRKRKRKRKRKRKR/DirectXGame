...
ComPtr<IDxcBlob> ShaderCompiler::CompileShader(const std::wstring& filePath, const wchar_t* profile) {
	...
	ExecuteCompile(filePath, profile, shaderResult);
...
void ShaderCompiler::ExecuteCompile(const std::wstring& filePath, const wchar_t* profile, IDxcResult*& shaderResult){
LPCWSTR arguments[] = {
		filePath.c_str(),
		L"-E", L"main",
		L"-T", profile,
