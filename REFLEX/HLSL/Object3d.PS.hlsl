...
cbuffer LightData : register(b0)
{
    ...
    // 有効な点光源・スポットライトのうち最大のindex+1（SceneLight::SetPointLight/SetSpotLightが
    // 更新する）。以前はgPad1（未使用の詰め物）だった16バイト行を転用し、以下のforループの
    // 上限をkMaxPointLights/kMaxSpotLights固定からこの値に変えることで、有効なライトが無い/
    // 少ない場面での毎ピクセルのループ回数を大幅に削減する
...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    if (gEnableLighting != 0)
    {
        ...
        // ループ上限はkMaxPointLights固定ではなく、実際に使っているぶんだけのgPointLightCount
        // にすることで、毎ピクセルの無駄なループ回数を削減する
        for (uint pi = 0; pi < gPointLightCount; ++pi)
        {
        ...
        for (uint si = 0; si < gSpotLightCount; ++si)
        {
